...
corrected_chan_file = '/Users/dhewitt/OneDrive - Nexus365/Analysis/TouchStudy/gtec62.sph';
...
for i = 1:length(cleaned_set_files)
    ...
    EEG = pop_select( EEG, 'rmchannel',{'LEar','REar','A1','A2'});
    ...
    EEG=pop_chanedit(EEG, 'changefield',{1,'labels','Fp1'},'changefield',{2,'labels','Fpz'},'changefield',{3,'labels','Fp2'}); % just incase it reverts

    [file_dir, file_name, ~] = fileparts(filename);
    %new_filename = fullfile([file_name, '_corrected_1604_noy.set']);
    new_filename = fullfile([file_name, '_corrected_nolobes.set']);
